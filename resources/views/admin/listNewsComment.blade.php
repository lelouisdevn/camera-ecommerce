  ...
  <div class="panel panel-default">
    ...
    <div class="table-responsive">
      <table class="table table-striped b-t b-light">
        ...
        @foreach ($Comment as $key => $ne)
          <tr>


              <td><label class="i-checks m-b-none"><input type="checkbox" name="post" value="{{ $ne->NewsId }}"><i></i></label></td>
            <td>{{ $ne->NewsId }}</td>
            <td width="200px;">{{$ne->NewsTitle}}</td>
            <td><img width="200px;" src="{{asset('/Uploads/News/'.$ne->NewsImage)}}"></td>
            <td>{{ $ne->UserName }}</td>
            <td width="450px;">
              {{$ne->NewsCommentContent}}
            </td>
            <!-- <td>
              {{$ne->NewsContent}}
            </td> -->

            <td width="200px;"><span class="text-ellipsis">{{$ne->NewsCommentTime}}</span></td>
